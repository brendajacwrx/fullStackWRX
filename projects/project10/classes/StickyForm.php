...
class StickyForm extends Validation {
	...
	public function validateForm($GlobalPost, $elementsArr){
		...
		if(strcmp($elementsArr['masterStatus']['status'], 'null')  === 0){
			$elementsArr['masterStatus']['status'] ='noerrors';
		}
		return $elementsArr;
	}
	...
	public function createOptions($elementsArr){
		$options = '';
		foreach($elementsArr['state']['options'] as $k=>$v){
			if($elementsArr['state']['selected'] == $k){
